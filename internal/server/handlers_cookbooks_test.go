...
func TestHandlers_Cookbooks(t *testing.T) {
	...
	t.Run("have cookbooks list preferred mode", func(t *testing.T) {
		...
		assertStringsInHTML(t, body, []string{
			...
			`<h2 class="card-title text-base w-[20ch] sm:w-[29ch] break-words">Lovely Canada</h2>`,
			`<h2 class="card-title text-base w-[20ch] sm:w-[29ch] break-words">Lovely America</h2>`,
			`<h2 class="card-title text-base w-[20ch] sm:w-[29ch] break-words">Lovely Ukraine</h2>`,
	...
	t.Run("create cookbook", func(t *testing.T) {
		...
		assertStringsInHTML(t, getBodyHTML(rr), []string{
			...
			`<img class="rounded-t-lg w-full border-b h-32 text-center object-cover max-w-48 md:h-48 hover:bg-gray-100 hover:opacity-80" src="/data/images/Placeholders/placeholder.cookbook.webp" onClick="__templ_cookbookImageClick`,
			`<form id="cookbook-image-form-1" enctype="multipart/form-data" hx-swap="none" hx-put="/cookbooks/1/image" hx-trigger="change from:#cookbook-image-1"><input id="cookbook-image-1" type="file" accept="image/*" name="image" required class="hidden" _="on drop or change make an FileReader called reader then if event.dataTransfer get event.dataTransfer.files[0] else get event.target.files[0] end then set {src: window.URL.createObjectURL(it)} on previous <img/> then remove .hidden from next <button/>"></form>`,
...
func TestHandlers_Cookbooks_Cookbook(t *testing.T) {
	...
	t.Run("cookbook belongs to user and is empty", func(t *testing.T) {
		...
		assertStringsInHTML(t, getBodyHTML(rr), []string{
			...
			`<div id="search_help" popover class="hidden card p-0 w-80 bg-base-100 shadow-xl max-h-[28rem] z-20 sm:w-[30rem] `,
			`<h2 class="card-title ">Search Help</h2>`,
			`<p class="text-xs mb-2">The following table provide examples of how to perform various searches. You may combine any of these in any order.</p>`,
			`<th>Multiple sources</th><td>src:allrecipes.com,tasteofhome.com</td>`,
...
func TestHandlers_Cookbooks_Share(t *testing.T) {
	...
	t.Run("access share link anonymous no recipes", func(t *testing.T) {
		...
		assertStringsInHTML(t, body, []string{
			...
			`<a class="btn btn-ghost text-lg" style="padding-left: 0" href="/"><img src="/data/images/Icons/android-chrome-192x192.png" alt="Logo" style="width: 2rem"> Recipya</a>`,
	...
	t.Run("access share link anonymous have recipes", func(t *testing.T) {
		...
		assertStringsInHTML(t, body, []string{
			...
			`<form hx-put="/cookbooks/1/reorder" hx-trigger="end" hx-swap="none"><input type="hidden" name="cookbook-id" value="1"><ul class="cookbooks-display grid gap-8 p-2 place-items-center text-sm md:p-0 md:text-base"><li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="3"><div class="indicator-item indicator-bottom badge badge-secondary cursor-none">1</div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 min-w-28 sm:w-32 sm:min-w-32"><img src="/data/images/Placeholders/placeholder.recipe.webp" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words">Gotcha</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">American</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/r/3?cookbook=1" hx-target="#content" hx-swap="innerHTML transition:true" hx-push-url="true">View</button></div></div></div></li></ul></form>`,
		})
