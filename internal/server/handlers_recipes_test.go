...
func TestHandlers_Recipes(t *testing.T) {
	...
	t.Run("user has recipes", func(t *testing.T) {
		...
		assertStringsInHTML(t, got, []string{
			...
			`<img class="h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the One recipe">`,
			`<img class="h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the Two recipe">`,
			`<img class="h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the Three recipe">`,
		})
...
func TestHandlers_Recipes_Edit(t *testing.T) {
	...
	t.Run("successful request", func(t *testing.T) {
		...
		assertStringsInHTML(t, getBodyHTML(rr), []string{
			...
			`<input type="text" name="tools" placeholder="1 frying pan" class="input input-bordered input-sm w-full" value="1 spoons" _="on keydown if event.key is 'Enter' halt the event then call addItem(event)">`,
	...
	t.Run("updated image", func(t *testing.T) {
		...
		if isImagesEqual || !isVideosEqual {
			t.Fatal("image should have been updated while videos stay unchanged")
		}
		if files.uploadImageHitCount == 0 {
	...
	t.Run("replaced image keeps order", func(t *testing.T) {
		firstID := uuid.New()
		secondID := uuid.New()
		replacementID := uuid.New()

		recipe := baseRecipe
		recipe.Images = []uuid.UUID{firstID, secondID}

		files := &mockFiles{
			uploadImageFunc: func(_ io.ReadCloser) (uuid.UUID, error) {
				return replacementID, nil
			},
		}
		srv.Files = files
		srv.Repository = &mockRepository{RecipesRegistered: map[int64]models.Recipes{1: {recipe}}}

		var form bytes.Buffer
		writer := multipart.NewWriter(&form)
		_ = writer.WriteField("media-managed", "1")
		_ = writer.WriteField("image-keep", firstID.String())

		emptySlotHeader := make(textproto.MIMEHeader)
		emptySlotHeader.Set("Content-Disposition", `form-data; name="images"; filename=""`)
		emptySlotHeader.Set("Content-Type", "image/webp")
		emptySlotPart, _ := writer.CreatePart(emptySlotHeader)
		_, _ = emptySlotPart.Write([]byte{})

		replacedSlotPart, _ := writer.CreateFormFile("images", "replace.jpg")
		_, _ = replacedSlotPart.Write([]byte("not a real file"))
		_ = writer.Close()

		rr := sendHxRequestAsLoggedIn(srv, http.MethodPut, fmt.Sprintf(uri, 1), header(writer.FormDataContentType()), strings.NewReader(form.String()))

		assertStatus(t, rr.Code, http.StatusNoContent)
		got, _ := srv.Repository.Recipe(recipe.ID, 1)
		if len(got.Images) != 2 {
			t.Fatalf("got %d images; want 2", len(got.Images))
		}
		if got.Images[0] != firstID || got.Images[1] != replacementID {
			t.Fatalf("got image order %v; want [%s %s]", got.Images, firstID, replacementID)
		}
	})
...
func TestHandlers_Recipes_View(t *testing.T) {
	...
	testcases2 := []struct {
	...
	}{
		{
			...
			videos: nil,
			want: []string{
				`<img style="object-fit: cover" alt="Image of the recipe" class="w-full max-h-80 md:max-h-[34rem]" src="/data/images/Placeholders/placeholder.recipe.webp">`,
			},
		},
		{
